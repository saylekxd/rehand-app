...
export interface AIAnalysisOutput {
  ...
  visualization: import('./visualization/types').OverlayState | null;
  performance: {
    visionFPS: number;
...
export class AICoordinator {
  ...
  async initialize(config: AICoordinatorConfig): Promise<void> {
    ...
    try {
      ...
      await this.hierarchicalPipeline.initialize({
        ...
        localLLMManager: this.localLLMManager || undefined,
        cloudLLMManager: this.cloudLLMManager || undefined,
        visualizationEngine: this.visualizationEngine || undefined
      });
  ...
  async processFrame(imageData: ImageData): Promise<AIAnalysisOutput> {
    ...
    try {
      ...
      if (this.visualizationEngine && result.vision && result.features && result.mlAnalysis && this.currentConfig) {
        try {
          visualizationData = await this.visualizationEngine.updateVisualization(
            result.vision,
            result.features,
            result.mlAnalysis,
